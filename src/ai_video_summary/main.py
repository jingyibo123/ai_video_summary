...
logger.remove()
# A default handler is set in main() based on CLI arguments
...
def main() -> None:
    ...
    parser.add_argument("--no-progress", action="store_true", help="关闭富文本进度条并启用 INFO 级别控制台日志（适用于后台或 MCP 调用）")
    ...
    # Configure console logger based on --no-progress
    if args.no_progress:
    ...
    try:
        ...
        if not args.skip_asr:
            try:
                if not agents.has_audio_track(video_path):
        ...
        with Progress(
            ...
            if not args.skip_asr:
                agents.extract_audio(video_path, aud_path, max_seconds=max_time)
            ...
            if (slides_info := _load_json(meta_path)) is not None:
                ...
                if unvalidated:
                    ...
                    with concurrent.futures.ThreadPoolExecutor(max_workers=config.vlm.max_workers) as pool:
                        futures = {
                            pool.submit(agents.vlm_task, config.vlm.base_url, config.vlm.api_key, config.vlm.model, "analyze", [output_dir / c['image'] if not Path(c['image']).is_absolute() else Path(c['image'])], config.vlm.supports_parse, config.vlm.supports_response_format, config.vlm.disable_thinking, config.vlm.max_thinking_tokens): c
        ...
        logger.exception(f"❌ 流水线意外崩溃！已中断。原因: {e}")
        sys.exit(1)
